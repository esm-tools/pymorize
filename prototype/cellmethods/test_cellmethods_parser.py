...
test_cases = [
    ...
    ("area: mean", [[("DIMENSION", "area"), ("ACTION", "mean")]]),
    (
        ...
        [
            [
                ("DIMENSION", "area"),
                ("ACTION", "mean"),
                ("SCOPE", "comment: over land and sea ice"),
            ],
...
def test_lexer_tokens():
    ...
    for token_type, values in test_tokens.items():
        for value in values:
            ...
            ), f"Expected token type {token_type} for {value}, got {token.type}"
            if token_type == "DIMENSION":
                assert (
                    token.value == value[:-1]
                ), f"Expected value {value[:-1]} for {value}, got {token.value}"
            elif token_type == "SCOPE":
                assert (
                    token.value == value[1:-1]
                ), f"Expected value {value[1:-1]} for {value}, got {token.value}"
            else:
                assert (
                    token.value == value
                ), f"Expected value {value}, got {token.value}"
